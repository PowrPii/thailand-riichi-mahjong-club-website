...
const NavigationMenu: React.FC<NavigationMenuProps> = ({ isOpen }) => {
    return (
        <div className={`z-[20] mt-20 py-5 fixed inset-0 bg-white bg-opacity-90 transform ${isOpen ? 'translate-y-0' : '-translate-y-full'} transition-transform duration-300 ease-in-out ${isOpen ? 'backdrop-blur' : ''}`} >
            <nav>
